...
function isStyle(arg: Rule[] | HNode): arg is Rule[] {
	if (Array.isArray(arg)) {
		for (const a of arg) {
			if (
				!Object.hasOwn(a, "selectorlist") ||
				!Object.hasOwn(a, "properties")
			) {
				return false;
			}
		}
	...
	return Object.entries(attribute)
		.map(([raw_key, value]) => {
			if (typeof value === "string") {
				const key = (raw_key === "className" ? "class" : raw_key).replace(
					"_",
					"-",
				);
		...
		if (typeof node !== "string") {
			if (isCompoundSelector(selector)) {
				...
				if (matchCompoundSelector(selector.compound, node)) {
					selectElementsCombinator(nodes, i, selector).map((e) =>
						result.add(e),
					);
				} else if (search_deep) {
...
function selectElementsCombinator(
	nodes: HNode[],
	index: number,
	...
	switch (selector.combinator) {
		case " ":
			selectElements(parent.child, selector.descendant, true).map((e) =>
				result.add(e),
			);
			selectElements(parent.child, selector, true).map((e) => result.add(e));
			break;
		case ">":
			selectElements(parent.child, selector.descendant).map((e) =>
				result.add(e),
			);
			break;
		case "+":
			if (nodes.length > index + 1) {
				...
				if (typeof next_node !== "string") {
					selectElements([next_node], selector.descendant).map((e) =>
						result.add(e),
					);
				}
			}
		...
		if (isCompoundSelector(selector)) {
			...
			if (matchCompoundSelector(selector.compound, result)) {
				result = {
					...
					style: result.style,
					child: result.child.map((e) =>
						insertElementsCombinator(e, selector, insert),
					),
				};
			} else if (search_deep) {
				result = {
					...
					style: result.style,
					child: result.child.map((e) =>
						insertElements(e, selector, insert, true),
					),
				};
			}
		}
...
function insertElementsCombinator(
	root: HNode,
	selector: ComplexSelector,
	insert: HNode[],
): HNode {
