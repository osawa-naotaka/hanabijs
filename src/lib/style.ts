...
export function style(
	...
	return rules.map(([selectors, properties]) => ({
		selectorlist: selectors.map((s) =>
			createSelector(s.length > 0 && s[0] === "&" ? s : ["*", " ", ...s]),
		),
		properties,
	}));
}
	...
	switch (selector.length) {
		...
		default:
			if (!tokenIsCombinator(selector[0]) && tokenIsCombinator(selector[1])) {
				return {
					compound: tokenIsCompoundSelector(selector[0])
						? selector[0]
						: [selector[0]],
...
function tokenIsCombinator(
	c: SimpleSelector | CompoundSelector | Combinator,
): c is Combinator {
	return (
		typeof c === "string" && (c === " " || c === ">" || c === "+" || c === "~")
	);
}

function tokenIsCompoundSelector(
	s: SimpleSelector | CompoundSelector | Combinator,
): s is CompoundSelector {
...
export function stringifyToCss(node: HNode): string {
	...
	return [
		rulesToString(node.style),
		node.child.map(stringifyToCss).join(""),
	].join("");
}
...
function propatiesToString(properties: Properties): string {
	return Object.entries(properties)
		.map(
			([key, value]) =>
				`${key.replace("_", "-")}: ${typeof value === "string" ? value : value.join(" ")};`,
		)
		.join(" ");
}
